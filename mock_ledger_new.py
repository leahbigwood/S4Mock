...
def create_mock_ledger_hp(outdir, healpix, mxxl, overwrite=False):    
    ...
    if outdir ==None:
        ...
        single_pixel_mxxl['TARGETID'] = np.zeros(nobj)
        ...
        hdu2 = fits.open(mtlfile)
        hdu_mtl = hdu2[1].data
        
        nobj = len(t)
        t['TARGETID'] = np.linspace(healpix*1000000,healpix*1000000+nobj+1,nobj,dtype=type(np.max(hdu_mtl['TARGETID'])))
